...
public class Database {
	...
	public static boolean addBatch(Context context, String batchcode,
			String course, String startdate,String classes,
			String period, String classesperweek, String remarks,boolean mon,
								   boolean tue,boolean wed,boolean thu,boolean fri,boolean sat,
								   String nomon,String notue,String nowed,String nothu,String nofri,String nosat,
	String stmon,String sttue,String stwed,String stthu,String stfri,String stsat) {
		...
		try {
			...
			addClasses(db, batchcode, startdate,classes, period,
					mon,tue,wed,thu,fri,sat,nomon,notue,nowed,nothu,nofri,nosat,
			stmon,sttue,stwed,stthu,stfri,stsat);
	...
	public static boolean updateBatch(Context context, String batchcode,
			String course,String period, String remarks) {
	...
	public static boolean updateClass(Context context, String classid,String classDate,String classTime, String period,String topics, String remarks) {
	...
	public static void addClasses(SQLiteDatabase db, String batchcode,
			String startdate,String classes, String period,
			boolean mon,boolean tue,boolean wed,
								  boolean thu,boolean fri,boolean sat,
								  String nomon,String notue,String nowed,String nothu
	,String nofri,String nosat,String stmon,String sttue,String stwed,String stthu,String stfri,String stsat) throws Exception {
		try {
            ...
            int Nomon=Integer.parseInt(nomon);
			int Notue=Integer.parseInt(notue);
			int Nowed=Integer.parseInt(nowed);
			int Nothu=Integer.parseInt(nothu);
			int Nofri=Integer.parseInt(nofri);
			int Nosat=Integer.parseInt(nosat);
			int classnumber = 1;
			...
			do {
				...
				switch (dow) {
					...
					case 2:
						...
						if (tue == true) {
							for(int i=1;i<=Notue;i++){
							values.put(Database.CLASSES_CLASSDATE, String.format(
									"%04d-%02d-%02d", c.get(Calendar.YEAR),
									c.get(Calendar.MONTH) + 1,
									c.get(Calendar.DAY_OF_MONTH)));
								values.put(Database.CLASSES_CLASSTIME, sttue);
							long rowid = db.insert(Database.CLASSES_TABLE_NAME, null,
									values);
							Log.d("CS", "Inserted into CLASSES" + rowid);
							classnumber++;
					...
					case 3:
						...
						if (wed == true) {
							for(int i=1;i<=Nowed;i++){
							values.put(Database.CLASSES_CLASSDATE, String.format(
									"%04d-%02d-%02d", c.get(Calendar.YEAR),
									c.get(Calendar.MONTH) + 1,
									c.get(Calendar.DAY_OF_MONTH)));
								values.put(Database.CLASSES_CLASSTIME, stwed);
							long rowid = db.insert(Database.CLASSES_TABLE_NAME, null,
									values);
							Log.d("CS", "Inserted into CLASSES" + rowid);
							classnumber++;
					...
					case 4:
						...
						if (thu == true) {
							for (int i = 1; i <= Nothu; i++) {
								values.put(Database.CLASSES_CLASSDATE, String.format(
										"%04d-%02d-%02d", c.get(Calendar.YEAR),
										c.get(Calendar.MONTH) + 1,
										c.get(Calendar.DAY_OF_MONTH)));
								values.put(Database.CLASSES_CLASSTIME, stthu);
								long rowid = db.insert(Database.CLASSES_TABLE_NAME, null,
										values);
								Log.d("CS", "Inserted into CLASSES" + rowid);
								classnumber++;
					...
					case 5:
						...
						if (fri == true) {
							for(int i=1;i<=Nofri;i++){
							values.put(Database.CLASSES_CLASSDATE, String.format(
									"%04d-%02d-%02d", c.get(Calendar.YEAR),
									c.get(Calendar.MONTH) + 1,
									c.get(Calendar.DAY_OF_MONTH)));
								values.put(Database.CLASSES_CLASSTIME, stfri);
							long rowid = db.insert(Database.CLASSES_TABLE_NAME, null,
									values);
							Log.d("CS", "Inserted into CLASSES" + rowid);
							classnumber++;
					...
					case 6:
						if(sat==true){
							for(int i=1;i<=Nosat;i++){
							values.put(Database.CLASSES_CLASSDATE, String.format(
									"%04d-%02d-%02d", c.get(Calendar.YEAR),
									c.get(Calendar.MONTH) + 1,
									c.get(Calendar.DAY_OF_MONTH)));
								values.put(Database.CLASSES_CLASSTIME, stsat);
							long rowid = db.insert(Database.CLASSES_TABLE_NAME, null,
									values);
							Log.d("CS", "Inserted into CLASSES" + rowid);
								classnumber++;
